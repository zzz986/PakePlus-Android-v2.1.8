        ...
        if type(v) == "string" then
            ...
            elseif rawget(root, v) or rawget(_G, v) then
                ...
                if (context.loader ~= nil) then
                    local project_path = context.loader.getImagePath(v)
                ...
                if v:find("%.9%.png") then
                    ...
                    setBackground(view, LuaBitmapDrawable(context.luaSupport, v))
                end
            end
...
local function loadlayout(t, root, group)
    ...
    if t.layout_behavior then
        if (t.layout_behavior == "appbar_scrolling_view_behavior") then
