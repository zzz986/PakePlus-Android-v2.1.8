...
dexes = luajava.astable(luacontext.luaDexLoader.getClassLoaders())
local libs = luacontext.luaDexLoader.getLibrarys()
...
local ThreadSupport = luajava.bindClass "net.fusionapp.core.util.ThreadSupport"
...
function _M.thread(src, ...)
    ...
    local luaThread
    if ... then
        luaThread = LuaThread(activity.luaSupport or service, src, true, Object { ... })
    else
        luaThread = LuaThread(activity.luaSupport or service, src, true)
    end
    luaThread.start()
...
function _M.task(src, ...)
    ...
    args[select("#", ...)] = nil
    local luaAsyncTask = LuaAsyncTask(activity.luaSupport, src, callback)
    luaAsyncTask.executeOnExecutor(LuaAsyncTask.THREAD_POOL_EXECUTOR, args)
    return luaAsyncTask
end

function _M.timer(f, d, p, ...)
    local luaTimer = LuaTimer(activity.luaSupport, f, Object { ... })
    if p == 0 then
        luaTimer.start(d)
    else
        luaTimer.start(d, p)
...
-- autoload module--
pcall(function()
    if (luacontext ~= nil and luacontext.getClass().getSimpleName() == "FusionActivity") then
        local nameList = luacontext.getAutoLoadModuleNameList()
